...
import { Upload, Download, Lock, LockKeyhole, Trash2, CheckCircle2, AlertCircle, Loader2, FileText, Settings2 } from 'lucide-react';
...
export default function PDFProtect() {
  const [items, setItems] = useState<ProtectItem[]>([]);
  const [password, setPassword] = useState('');
  const [isProcessingAll, setIsProcessingAll] = useState(false);

  const onDrop = useCallback((acceptedFiles: File[]) => {
    const newItems: ProtectItem[] = acceptedFiles.map(file => ({
  ...
  const protectItem = async (id: string) => {
    const item = items.find(i => i.id === id);
    if (!item || item.status === 'processing' || !password) return;

    setItems(prev => prev.map(i => i.id === id ? { ...i, status: 'processing' } : i));

    try {
      const arrayBuffer = await item.original.arrayBuffer() as ArrayBuffer;
  ...
  const downloadAll = () => {
    items.forEach(item => {
      if (item.processed) {
        ...
        link.download = `protected_${item.original.name}`;
        link.click();
      }
    });
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 py-8 px-4">
      <div className="text-center space-y-4">
        <h1 className="text-4xl font-extrabold text-white tracking-tight">Batch PDF Protect</h1>
        <p className="text-slate-400">Restructure and pseudo-encrypt multiple PDF documents simultaneously.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="bg-slate-800/30 border border-white/5 rounded-3xl p-6 space-y-8 sticky top-24">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold flex items-center gap-2">
                <LockKeyhole className="w-5 h-5 text-red-400"/> Vault Config
              </h3>
              {items.length > 0 && (
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-300">Global Security Password</label>
                <input 
                  type="password" 
                  value={password} 
                ...
                <p className="text-[10px] text-slate-500 leading-relaxed mt-2">
                  This password will be applied to all files in the queue. Processing happens locally in your browser.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-white/10">
              <button 
                ...
                disabled={items.length === 0 || isProcessingAll || !password}
              ...
              >
                {isProcessingAll ? "Securing PDFs..." : "Protect All PDFs"}
              </button>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          >
            ...
            <p className="text-slate-300 font-medium text-lg">Batch Upload for PDF Protection</p>
            <p className="text-xs text-slate-500">Secure multiple sensitive documents at once</p>
          </div>

          <div className="bg-black/40 border border-white/5 rounded-3xl p-6 min-h-[500px]">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/5">
              <h4 className="text-xs font-bold uppercase tracking-widest text-slate-500">Protection Queue</h4>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        {item.status === 'done' && item.processed && (
                          <a 
                            href={item.processed} download={`protected_${item.original.name}`}
